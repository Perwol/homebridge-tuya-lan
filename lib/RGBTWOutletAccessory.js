...
class RGBTWOutletAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.colorFunction = this.device.context.colorFunction && this.device.context.colorFunction.toUpperCase();
        if (!this.colorFunction && dps[this.dpColor]) {
            this.colorFunction = {12: 'HSB', 14: 'HEXHSB'}[dps[this.dpColor].length];
            if (this.colorFunction) console.log(`[TuyaAccessory] Color format for ${this.device.context.name} (${this.device.context.version}) identified as ${this.colorFunction}.`);
        }
        if (!this.colorFunction) {
            this.colorFunction = 'Unknown';
            console.log(`[TuyaAccessory] Color format for ${this.device.context.name} (${this.device.context.version}) is unknown.`);
        }
