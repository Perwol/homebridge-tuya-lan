...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.mqttClient.sendStatus('test', 'elo');
    }
...
//         });
    ...
    nightModeUpdated(nightModeReqString, self) {
        const currentStatus = self.nightModeStatus;
        const nightModeReq = parseInt(nightModeReqString);
        if (! nightModeReq.length) {
            return;
        }
        console.log('ppp0');
        if (nightModeReq === currentStatus) {
            return;
        }

        console.log('ppp1');
        const nightModeReqBool = nightModeReq == 1 ? true : false;
        console.log(nightModeReqBool);
        if (this.currentState == 0) {
            if (this.currentSetState == 1) {
                this.pendingCmdDps1 = '103';
                this.pendingCmdValue1 = nightModeReqBool;
