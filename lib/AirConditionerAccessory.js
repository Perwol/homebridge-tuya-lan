...
class MqttHandler {
    ...
    connect() {
        ...
        this.mqttClient.on('connect', function () {
            this.mqttClient.publish(deviceName + '/online', 'online', { retain:true, qos:1 });
            // client.subscribe('milight/command/#', function (err) {
            //     if (! err) {
            // 		avaliableHandlers.forEach(function(element) {
            // 			handlersTimeLeft[element] = 0;
...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.mqttClient = MqttHandler(this.device.context.name, 'mqtt://' + this.device.context.mqttHost, this.mqttOptions);
