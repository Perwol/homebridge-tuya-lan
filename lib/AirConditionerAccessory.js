...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.mqttClient = mqtt.connect('mqtt://' + this.device.context.mqttHost, this.mqttOptions);
    ...
    _registerCharacteristics(dps) {
        ...
        this.mqttClient.on('connect', function () {
            this.mqttClient.publish(this.device.context.name + '/online', 'online', { retain:true, qos:1 });
            // client.subscribe('milight/command/#', function (err) {
            //     if (! err) {
            // 		avaliableHandlers.forEach(function(element) {
            // 			handlersTimeLeft[element] = 0;
