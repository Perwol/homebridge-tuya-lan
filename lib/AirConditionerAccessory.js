...
class AirConditionerAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        fs.watchFile(statusNightModeFile, { interval: 1000 }, (curr, prev) => {
            const nightModeReq = fs.readFileSync(statusNightModeFile);

            console.log('zzz');
            console.log(nightModeReq.toString());
        ...
        fs.watchFile(statusTimerFile, { interval: 1000 }, (curr, prev) => {
            const statusTimerReq = parseInt(fs.readFileSync(statusTimerFile));
    ...
    getTimer(callback) {
        this.getState('105', (err, dp) => {
            ...
            fs.writeFile(statusNightModeFile, this.currentTimer, function(err) {});
        });
    }
