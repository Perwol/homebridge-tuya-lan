...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.pendingCmdDps = null;
        this.pendingCmdValue = null;
    ...
    _registerCharacteristics(dps) {
        ...
        fs.watchFile(statusNightModeFile, { interval: 100 }, (curr, prev) => {
            ...
            console.log('nightModeReq');
            console.log(nightModeReq);
            console.log(this.nightModeStatus);
            // this.delay(10000);
            if (nightModeReq == this.nightModeStatus) {
                return;
            }
            console.log('nightModeReqEnd');
            ...
            if (this.currentState == 0) {
                ...
                if (this.currentSetState == 1) {
                    console.log('kkk');
                    this.pendingCmdDps = '103';
                    this.pendingCmdValue = nightModeReqBool;
                    return;
                } else {
                    this.nightModeStatus = 0;
    ...
    _getActive(dp) {
        ...
        if (this.pendingCmdDps) {
            console.log('pend');

            var self = this;
            setTimeout(function () {
                self.setState(self.pendingCmdDps, self.pendingCmdValue, null);
                self.pendingCmdDps = null;
                self.pendingCmdValue = null;
            },100);
        }
