...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.mqttOptions = {
            ...
            will: {
                topic: this.device.context.name + '/online',
                payload: 'offline',
                qos: 1,
                retain: true,
            }
        ...
        this.mqttClient = new MqttHandler(this.device.context.name, 'mqtt://' + this.device.context.mqttHost, this.mqttOptions);
...
//                 fs.writeFile(statusNightModeFile, this.nightModeStatus, function(err) {});
            ...
            if (changes.hasOwnProperty('112')) {
                ...
                this.mqttClient.sendStatus('humidity', this.currentHumidity.toString());
