...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.cmdCoolStatus = '1';
        this.cmdHeatStatus = '2';
        this.cmdAutoStatus = 'AUTO';

        this.cmdCoolStatus = 'COOL';
        if (this.device.context.cmdCoolStatus) {
            if (/^c[a-z]+$/i.test(this.device.context.cmdCoolStatus)) this.cmdCoolStatus = ('' + this.device.context.cmdCoolStatus).trim();
            else throw new Error('The cmdCoolStatus doesn\'t appear to be valid: ' + this.device.context.cmdCoolStatus);
        }

        this.cmdHeatStatus = 'HEAT';
        if (this.device.context.cmdHeatStatus) {
            if (/^h[a-z]+$/i.test(this.device.context.cmdHeatStatus)) this.cmdHeatStatus = ('' + this.device.context.cmdHeatStatus).trim();
            else throw new Error('The cmdHeatStatus doesn\'t appear to be valid: ' + this.device.context.cmdHeatStatus);
        }

        this.cmdAutoStatus = 'AUTO';
        if (this.device.context.cmdAutoStatus) {
            if (/^a[a-z]+$/i.test(this.device.context.cmdAutoStatus)) this.cmdAutoStatus = ('' + this.device.context.cmdAutoStatus).trim();
            else throw new Error('The cmdAutoStatus doesn\'t appear to be valid: ' + this.device.context.cmdAutoStatus);
        }
    ...
    _getCurrentHeaterCoolerState(dps) {
        ...
        switch (dps['101']) {
            case this.cmdCoolStatus:
                return Characteristic.CurrentHeaterCoolerState.COOLING;

            case this.cmdHeatStatus:
    ...
    setTargetHeaterCoolerState(value, callback) {
        ...
        switch (value) {
            case Characteristic.TargetHeaterCoolerState.COOL:
                ...
                return this.setState('101', this.cmdCoolStatus, callback);

            case Characteristic.TargetHeaterCoolerState.HEAT:
                if (this.device.context.noHeat) return callback();
                return this.setState('101', this.cmdHeatStatus, callback);

            case Characteristic.TargetHeaterCoolerState.AUTO:
                if (this.device.context.noAuto) return callback();
                return this.setState('101', this.cmdAutoStatus, callback);
        }
