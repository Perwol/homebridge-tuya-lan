...
class AirConditionerAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty('1') || changes.hasOwnProperty('103') || changes.hasOwnProperty('105')) {
                this.cmdWaiting = false;
                clearTimeout(this.cmdWaitingTimer);
            }
        ...
        fs.watchFile(statusNightModeFile, { interval: 1000 }, (curr, prev) => {
            ...
            while (this.cmdWaiting) {
                this.delay(10);
            }

            this.cmdWaiting = true;
            var self = this;
            this.cmdWaitingTimer = setTimeout(function() {
                self.cmdWaiting = false;
            }, 3000);
