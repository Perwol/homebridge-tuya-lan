...
class AirConditionerAccessory extends BaseAccessory {
    ...
    _getCurrentHeaterCoolerState(dps) {
        ...
        if (!dps['1']) {
            this.currentCmd = Characteristic.CurrentHeaterCoolerState.INACTIVE;
            return Characteristic.CurrentHeaterCoolerState.INACTIVE;
        }

        switch (dps['101']) {
            case this.cmdCool:
                this.currentCmd = Characteristic.CurrentHeaterCoolerState.COOLING;
                return Characteristic.CurrentHeaterCoolerState.COOLING;

            case this.cmdHeat:
                this.currentCmd = Characteristic.CurrentHeaterCoolerState.HEATING;
                return Characteristic.CurrentHeaterCoolerState.HEATING;

            default:
                this.currentCmd = Characteristic.CurrentHeaterCoolerState.IDLE;
        ...
        if (this.currentCmd !== value) {
            ...
            switch (value) {
                case Characteristic.TargetHeaterCoolerState.COOL:
                    ...
                    return this.setMultiState({'1': true, '101': this.cmdCool}, callback);

                case Characteristic.TargetHeaterCoolerState.HEAT:
                    if (this.device.context.noHeat) return callback();
                    return this.setMultiState({'1': true, '101': this.cmdHeat}, callback);

                case Characteristic.TargetHeaterCoolerState.AUTO:
                    if (this.device.context.noAuto) return callback();
                    return this.setMultiState({'1': true, '101': this.cmdAuto}, callback);
            }


        }
        callback();
            // this.setMultiState({'1': true, '104': this.convertRotationSpeedFromHomeKitToTuya(value)}, callback);
    ...
    _getRotationSpeed(dps) {
        ...
        console.log(this.convertRotationSpeedFromTuyaToHomeKit(dps['104']));
