...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        if (this.device.context.cmdCool) {
            ...
            else throw new Error('The cmdCoolStatus doesn\'t appear to be valid: ' + this.device.context.cmdCool);
        }

        this.cmdHeat = 'HEAT';
        if (this.device.context.cmdHeat) {
            if (/^h[a-z]+$/i.test(this.device.context.cmdHeat)) this.cmdHeat = ('' + this.device.context.cmdHeat).trim();
            else throw new Error('The cmdHeatStatus doesn\'t appear to be valid: ' + this.device.context.cmdHeat);
        }

        this.cmdAuto = 'AUTO';
        if (this.device.context.cmdAuto) {
            if (/^a[a-z]+$/i.test(this.device.context.cmdAuto)) this.cmdAuto = ('' + this.device.context.cmdAuto).trim();
            else throw new Error('The cmdAutoStatus doesn\'t appear to be valid: ' + this.device.context.cmdAuto);
        }
